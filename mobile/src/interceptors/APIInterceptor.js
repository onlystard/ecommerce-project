    ...
    error => {
      ...
      if (errorRes) {
        ...
        showError({
          ...
          error_description: errorRes.data.error_description || {},
        });
      } else {
        let isConn = true;
...
function showError({ error = {}, status, error_description = {} }) {
  let message = '';
  let title = i18n.t('FinCCP::CcpError.Msg');

  if (error === 'invalid_grant' && error_description === 'Invalid username or password!') {
    ...
    title = i18n.t('AbpIdentity::Volo.Abp.Identity:UserLockedOut');
    message = i18n.t('AbpAccount::AccountCollectorLock');
